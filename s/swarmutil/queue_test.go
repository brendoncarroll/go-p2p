...
func TestQueue(t *testing.T) {
	...
	const N = 3
	q := NewQueue[p2p.Addr](N, 1<<16)
	defer q.Close()

	for i := 0; i < N; i++ {
		require.Equal(t, i, q.Len())
		require.True(t, q.Deliver(p2p.Message[p2p.Addr]{
			Payload: []byte("hello world"),
		}))
		require.Equal(t, i+1, q.Len())
	}
	for i := 0; i < 2; i++ {
		require.False(t, q.Deliver(p2p.Message[p2p.Addr]{
			Payload: []byte("goodbye world"),
		}))
	}
	for i := 0; i < N; i++ {
		require.Equal(t, N-i, q.Len())
		err := q.Receive(ctx, func(m p2p.Message[p2p.Addr]) {
...
func BenchmarkQueue(b *testing.B) {
	b.Run("SendRecv", func(b *testing.B) {
		ctx := context.Background()
		const msgSize = 1 << 15
		q := NewQueue[p2p.Addr](1, msgSize)

		msgIn := p2p.Message[p2p.Addr]{
		...
		for i := 0; i < b.N; i++ {
			...
			if err := q.Receive(ctx, func(msg p2p.Message[p2p.Addr]) {
