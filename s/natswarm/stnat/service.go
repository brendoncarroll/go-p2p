...
func DefaultParams() Params {
	return Params{
		NATRenewal: 30 * time.Minute,
		NATTimeout: 5 * time.Second,
		NATLease:   10 * time.Second,
	}
}
...
func (s *Service) process(ctx context.Context) int {
	...
	nats := discoverAll(ctx, s.params.NATRenewal, s.params.NATTimeout)
...
func (s *Service) updateMapping(ctx context.Context, mapping *Mapping, nats map[string]Device, renew bool) {
	...
	renewalTime := s.params.NATRenewal
